...
# Add data augmentation and remove randomness in augmentation
class Agent():
    """
    Agent for training
    """
    ...
    def save_param(self):
        torch.save(self.net.state_dict(), '/home/mila/l/lea.cote-turcotte/LUSR/checkpoints/policy_augm.pt')
    ...
    def update(self):
        ...
        for _ in range(self.ppo_epoch):
            for index in BatchSampler(SubsetRandomSampler(range(self.buffer_capacity)), self.batch_size, False):
                ...
                imgs = RandomTransform(s[index]).apply_transformations(nb_class=4)
                imgs = imgs.reshape(-1, *imgs.shape[2:]) 
                save_image(imgs, "/home/mila/l/lea.cote-turcotte/LUSR/figures/augm.png")

                alpha, beta = self.net(imgs)[0]
                dist = Beta(alpha, beta)
...
if __name__ == "__main__":
    ...
    results_ppo = Resutls(title="Moving averaged episode reward", xlabel="episode", ylabel="augm_running_score")
    results_ppo.create_logs(labels=["episode", "augm_running_score", "augm_episode_score", "training_time"], init_values=[[], [], [], []])

    agent = Agent(args)
    env = Env()

    augm_running_score = 0
    training_time = 0
    state = env.reset()
    start_time = time.time()
    # 100000
    for i_ep in range(10000):
        augm_episode_score = 0
        score = 0
        episode_lenght = 0
        state = env.reset()
        ...
        for t in range(1000):
            ...
            if done or die:
                break
        augm_episode_score = score
        augm_running_score = augm_running_score * 0.99 + score * 0.01

        if i_ep % args.log_interval == 0:
            training_time = time.time() - start_time
            results_ppo.update_logs(["episode", "augm_running_score", "augm_episode_score", "training_time"], [i_ep, augm_running_score, augm_episode_score, training_time])
            print('Ep {}\tLast score: {:.2f}\tMoving average score: {:.2f}'.format(i_ep, score, augm_running_score))
            print('Training time: {:.2f}\t'.format(training_time))
            agent.save_param()
            results_ppo.save_logs('/home/mila/l/lea.cote-turcotte/LUSR/logs', str(6))
        if augm_running_score > env.reward_threshold:
            results_ppo.save_logs('/home/mila/l/lea.cote-turcotte/LUSR/logs', str(6))
            results_ppo.generate_plot('/home/mila/l/lea.cote-turcotte/LUSR/logs/6','/home/mila/l/lea.cote-turcotte/LUSR/figures')
            print("Solved! Running reward is now {} and the last episode runs to {}!".format(augm_running_score, score))
            break
    results_ppo.save_logs('/home/mila/l/lea.cote-turcotte/LUSR/logs', str(6))
    results_ppo.generate_plot('/home/mila/l/lea.cote-turcotte/LUSR/logs/6', '/home/mila/l/lea.cote-turcotte/LUSR/figures')