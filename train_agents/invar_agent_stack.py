...
from CDARL.utils import Results
import cv2
from torchvision.utils import save_image
from CDARL.utils import RandomTransform, ExpDataset
from torch.utils.data import Dataset, DataLoader
from torchvision import transforms
import time

parser = argparse.ArgumentParser(description='Train a invariant ppo agent for the CarRacing-v0')
parser.add_argument('--gamma', type=float, default=0.99, metavar='G', help='discount factor (default: 0.99)')
parser.add_argument('--encoder-path', default='/home/mila/l/lea.cote-turcotte/CDARL/checkpoints/encoder_endtoend_stack.pt')
...
class Encoder(nn.Module):
    def __init__(self, content_size = 32, input_channel = 12):
        ...
        self.linear_mu = nn.Linear(2*2*256, content_size)
...
class Agent():
    ...
    def save_param(self):
        torch.save(self.net.state_dict(), '/home/mila/l/lea.cote-turcotte/CDARL/checkpoints/policy_invar_stack.pt')
...
if __name__ == "__main__":
    ...
    results_ppo = Results(title="Moving averaged episode reward", xlabel="episode", ylabel="invariant_repr_running_score")
    results_ppo.create_logs(labels=["episode", "invariant_repr_running_score", "invariant_repr_episode_score", "training_time"], init_values=[[], [], [], []])
    ...
    for i_ep in range(10000):
        invariant_repr_episode_score = 0
        score = 0
        episode_lenght = 0
        state = env.reset()
        ...
        for t in range(1000):
            ...
            if done or die:
                break
        invariant_repr_episode_score = score
        invariant_repr_running_score = invariant_repr_running_score * 0.99 + score * 0.01

        if i_ep % args.log_interval == 0:
            training_time = time.time() - start_time
            results_ppo.update_logs(["episode", "invariant_repr_running_score", "invariant_repr_episode_score", "training_time"], [i_ep, invariant_repr_running_score, invariant_repr_episode_score, training_time])
            ...
            agent.save_param()
            results_ppo.save_logs('/home/mila/l/lea.cote-turcotte/CDARL/logs', str(13))
        if invariant_repr_running_score > env.reward_threshold:
            results_ppo.save_logs('/home/mila/l/lea.cote-turcotte/CDARL/logs', str(13))
            results_ppo.generate_plot('/home/mila/l/lea.cote-turcotte/CDARL/logs/3','/home/mila/l/lea.cote-turcotte/CDARL/figures')
            print("Solved! Running reward is now {} and the last episode runs to {}!".format(invariant_repr_running_score, score))
            break
    results_ppo.save_logs('/home/mila/l/lea.cote-turcotte/CDARL/logs', str(13))
    results_ppo.generate_plot('/home/mila/l/lea.cote-turcotte/CDARL/logs/13', '/home/mila/l/lea.cote-turcotte/CDARL/figures')