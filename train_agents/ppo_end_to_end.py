...
from CDARL.utils import RandomTransform, Results, create_logs, seed_everything

parser = argparse.ArgumentParser(description='Train a PPO agent for the CarRacing-v0')
parser.add_argument('--algo', default='ppo_end_to_end', type=str)
...
parser.add_argument('--vae-update-freq', default=1, type=int)
...
class ActorCriticNet(nn.Module):
    def __init__(self, encoder):
        ...
        self.main = encoder
        self.critic = nn.Sequential(nn.Linear(encoder.out_dim, 400), nn.ReLU(), nn.Linear(400, 300), nn.ReLU(), nn.Linear(300, 1))
        self.actor = nn.Sequential(nn.Linear(encoder.out_dim, 400), nn.ReLU(), nn.Linear(400, 300), nn.ReLU())
...
class Agent():
    ...
    def __init__(self, args, results_vae, vae_batches, vae_epoch):
        ...
        self.net = ActorCriticNet(self.model.encoder).to(device)
            ...
            for _ in range(100):
                for index in BatchSampler(SubsetRandomSampler(range(self.buffer_capacity)), self.batch_size_vae, False):
                    ...
                    for i_class in range(imgs.shape[0]):
                        ...
                        save_image(image[:, :3, :, :], "/home/mila/l/lea.cote-turcotte/CDARL/checkimages/class_%d.png" % (i_class))
                ...
                if self.vae_batches % 10000 == 0:
                    ...
                    torch.save(self.model.encoder.state_dict(), os.path.join(log_dir, "encoder_endtoend_stack.pt"))
            self.vae_epoch += 1
