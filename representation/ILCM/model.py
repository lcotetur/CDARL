...
from CDARL.representation.ILCM.nets import make_mlp, make_elementwise_mlp, Encoder
from CDARL.representation.ILCM.experiment_utils import mask, clean_and_clamp
...
def gaussian_log_likelihood(x, mean, std, full=True, reduction="sum"):
    ...
    if reduction == "sum":
        ...
        log_likelihood = torch.mean(log_likelihood, dim=tuple(range(1, len(log_likelihood.shape)))).unsqueeze(1)
    elif reduction == "none":
        pass
    else:
